...
import { Home, Calendar, List, BookOpen, PlusCircle, Settings, ChevronLeft, ChevronRight } from 'lucide-react';
...
const LeftSidebar = ({ user, isCollapsed = false }: LeftSidebarProps) => {
  const avatarUrl = user.avatar_url
    ? user.avatar_url.startsWith('http') ? user.avatar_url : `http://localhost:8000${user.avatar_url}`
    : null;

  const sidebarWidth = isCollapsed ? 56 : 270;

  return (
    <aside className="fixed top-0 left-0 h-screen flex flex-col z-40 border-r transition-all duration-300"
      style={{ width: sidebarWidth, background: 'var(--mdf-bg)', borderColor: 'var(--border)' }}>
      ...
      <div className="px-4 py-3 border-t space-y-3" style={{ borderColor: 'var(--border)' }}>
        ...
        <button
          ...
          className="w-full flex items-center justify-center p-2 text-white/50 hover:text-white transition-colors"
