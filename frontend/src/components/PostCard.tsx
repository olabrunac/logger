          ...
          {isEditing ? (
            <div className="mt-1">
              ...
              <div className="flex items-center justify-between mt-1.5">
                <div className="text-[10px] text-right" style={{ color: editText.length > 260 ? '#f87171' : 'var(--text-dim)' }}>
                  {editText.length}/280
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={cancelEdit} className="flex items-center gap-1 text-xs text-white/50 hover:text-white/80 transition-colors" disabled={savingEdit}>
                    <X size={12} /> Cancelar
                  </button>
                  <button onClick={saveEdit} disabled={!editText.trim() || savingEdit} className="flex items-center gap-1 text-xs mdf-btn-primary disabled:opacity-30">
