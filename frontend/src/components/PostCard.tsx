  ...
  return (
    <div className="mdf-card rounded-xl overflow-hidden">
      <div className="p-4 pb-3 flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <Link to={`/profile/${post.username}`} className="text-base font-bold text-white hover:underline">{post.username}</Link>
            <span className="text-base text-white/30">{timeAgo(post.created_at)}</span>
          ...
          {isEditing ? (
            <div className="mt-1">
              <textarea
                ...
                rows={3}
                className="w-full bg-[var(--mdf-surface)] border border-white/10 focus:border-[var(--accent)] outline-none rounded-lg px-3 py-2 text-base text-white placeholder:text-white/25 resize-none transition-colors"
              />
              <div className="flex items-center justify-between mt-1.5">
                <div className="text-base text-right" style={{ color: editText.length > 260 ? '#f87171' : 'var(--text-dim)' }}>
                  {editText.length}/280
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={cancelEdit} className="flex items-center gap-1 text-base text-white/50 hover:text-white/80 transition-colors" disabled={savingEdit}>
                    <X size={12} /> Cancelar
                  </button>
                  <button onClick={saveEdit} disabled={!editText.trim() || savingEdit} className="flex items-center gap-1 text-base mdf-btn-primary disabled:opacity-30">
            ...
            <p className="text-base text-white/80 mt-1 whitespace-pre-wrap break-words"><HashtagText text={post.content} /></p>
          )}
        </div>
      </div>
      ...
      <div className="px-4 py-2 flex items-center gap-4 border-t" style={{ borderColor: 'var(--border)' }}>
        ...
        <button onClick={() => onLike(post.id)} className="flex items-center gap-1.5 text-base transition-colors" style={{ color: post.is_liked ? 'var(--accent)' : 'rgba(255,255,255,0.4)' }}>
