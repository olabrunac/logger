          ...
          {logs.length > 0 ? (
            <div className="flex gap-1.5 overflow-x-auto pb-1">
              {[...logs]
                ...
                .slice(0, 5)
                .map(log => (
                  <Link key={log.id} to={getLogUrl(log.media_item)} className="w-[48px] h-[66px] rounded-md overflow-hidden flex-shrink-0 relative group border" style={{ borderColor: 'var(--border)', borderBottom: '5px solid ' + (MEDIA_COLORS[log.media_item.media_type] || '#666') }} title={log.media_item.title}>
