              ...
              <div className="flex gap-3 items-start">
                {coverUrl ? (
                  <div className="w-14 h-20 rounded-lg overflow-hidden flex-shrink-0" style={{ borderBottom: '3px solid ' + meta.color }}>
                    <img src={coverUrl} alt={event.media_item.title} loading="lazy" decoding="async" className="w-full h-full object-cover" />
                  </div>
                ) : (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-sm text-white/30 mt-0.5">{epLabel}</div>
                </div>
              </div>
            </Link>
              ...
              <div className="mt-2">
                ...
                {event.review_text && (
                  <div className="text-sm text-white/60 leading-relaxed">
                    ...
                    {reviewTruncated && (
                      <button onClick={() => setExpandedReview(!expandedReview)} className="text-sm ml-1 hover:underline" style={{ color: 'var(--accent)' }}>
      ...
      {showReplies && (
        <div className="border-t px-4 py-3 space-y-2" style={{ borderColor: 'var(--border)' }}>
          {loadingReplies ? (
            ...
            <div className="text-sm text-white/20">Nenhuma resposta</div>
          ) : (
            replies.map(r => (
              <div key={r.id} className="flex gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <span className="text-sm font-semibold mr-1" style={{ color: 'var(--accent)' }}>{r.username}</span>
                  <span className="text-sm text-white/60">{r.content}</span>
                </div>
                <span className="text-sm text-white/20 flex-shrink-0">{timeAgo(r.created_at)}</span>
              </div>
            ))
          )}
          <div className="flex gap-2 mt-2">
            <input
              ...
              className="flex-1 bg-white/5 rounded-full px-3 py-1.5 text-sm text-white placeholder:text-white/20 outline-none border-none"
            />
